...
def get_datasplit(val_ids, data_path = "data"):
    data = pd.read_csv(os.path.join(data_path, "structured_data_train.csv"))
...
def get_omega(data, split=10):
    ...
    for entry in data[train_inds]:
        ...
        row = int(field[0][1:])
        col = int(field[1][1:])
        omega_train.append([row-1, col-1, entry[1]])
    ...
    for entry in data[test_inds]:
        ...
        row = int(field[0][1:])
        col = int(field[1][1:])
        omega_test.append([row-1, col-1, entry[1]])
    omega_test = np.asarray(omega_test)
...
def make_submission(res_matrix, path="data/sampleSubmission.csv", out_path="results/submission.csv"):
